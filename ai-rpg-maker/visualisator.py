...
def load_relations():
    if not os.path.exists(RELATIONS_FILE):
        print(f"Nie znaleziono pliku {RELATIONS_FILE}")
        return []
...
def visualize(relations):
    ...
    plt.legend(title="Typ relacji", loc="upper left", fontsize=12)

    plt.title("Graf relacji NPC - czytelna wersja", fontsize=16)
    plt.axis("off")
    plt.tight_layout()

    plt.savefig(GRAPH_OUTPUT_FILE, dpi=300)
    print(f"Graf zapisany jako {GRAPH_OUTPUT_FILE}")
    plt.show()
